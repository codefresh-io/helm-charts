...
{{- $pvcName := printf "%s-%s" (include "cf-common-0.8.0.names.fullname" $) $pvcIndex }}

{{- if and (hasKey $pvcItem "nameOverride") $pvcItem.nameOverride  }}
{{- $pvcName = include "cf-common-0.8.0.tplrender" (dict "Values" $pvcItem.nameOverride "context" $) }}
{{- end }}
---
...
metadata:
  ...
  {{- if $pvcItem.labels }}
  {{- include "cf-common-0.8.0.tplrender" (dict "Values" $pvcItem.labels "context" $) | nindent 4 }}
  {{- end }}
  annotations:
  {{- if $pvcItem.retain }}
  ...
  {{- include "cf-common-0.8.0.tplrender" (dict "Values" $pvcItem.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  ...
  {{ include "cf-common-0.8.0.storageclass" ( dict "persistence" $pvcItem "context" $) }}
