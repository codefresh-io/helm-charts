...
{{- include "cf-common.v0.0.24.ingress" . -}}
*/}}
{{- define "cf-common.v0.0.24.ingress" -}}

{{- range $ingressIndex, $ingressItem := .Values.ingress }}

{{- $ingressName := printf "%s-%s" (include "cf-common.v0.0.24.names.fullname" $) $ingressIndex  -}}
...
metadata:
  ...
  {{- include "cf-common.v0.0.24.tplrender" (dict "Values" $ingressItem.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if $ingressItem.annotations }}
  annotations: {{- include "cf-common.v0.0.24.tplrender" (dict "Values" $ingressItem.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  ...
  tls:
    ...
    - hosts:
        {{- range .hosts }}
        - {{ include "cf-common.v0.0.24.tplrender" (dict "Values" . "context" $) | quote }}
        {{- end }}
      {{- if .secretName }}
      secretName: {{ include "cf-common.v0.0.24.tplrender" (dict "Values" .secretName "context" $) | quote}}
      {{- end }}
    {{- end }}
  {{- end }}
  rules:
  ...
  {{- range $ingressItem.hosts }}
    - host: {{ include "cf-common.v0.0.24.tplrender" (dict "Values" .host "context" $) | quote }}
      http:
        paths:
          ...
          {{- end }}
          {{- range .paths }}
          - path: {{ include "cf-common.v0.0.24.tplrender" (dict "Values" .path "context" $) | quote }}
            ...
            backend:
              service:
                name: {{ include "cf-common.v0.0.24.tplrender" (dict "Values" $serviceName "context" $) }}
                port:
                  number: {{ include "cf-common.v0.0.24.tplrender" (dict "Values" $servicePort "context" $) }}
          {{- end }}
  {{- end }}
