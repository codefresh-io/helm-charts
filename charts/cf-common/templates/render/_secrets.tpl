...
*/}}
{{- define "cf-common-0.14.0.secrets" -}}

{{- range $secretIndex, $secretItem := .Values.secrets }}

{{- if $secretItem.enabled }}
{{- $secretName := printf "%s-%s" (include "cf-common-0.14.0.names.fullname" $) $secretIndex }}
---
apiVersion: v1
kind: Secret
metadata:
  ...
  {{- include "cf-common-0.14.0.tplrender" (dict "Values" $secretItem.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if $secretItem.annotations }}
  annotations: {{- include "cf-common-0.14.0.tplrender" (dict "Values" $secretItem.annotations "context" $) | nindent 4 }}
  {{- end }}
...
{{- if $secretItem.stringData }}
  ...
  {{- end }}
stringData: {{ include "cf-common-0.14.0.tplrender" (dict "Values" $secretItem.stringData "context" $) | nindent 2 }}
