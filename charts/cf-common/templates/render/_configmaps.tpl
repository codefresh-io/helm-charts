...
{{- define "cf-common-0.34.0.configmaps" -}}

{{- range $configMapIndex, $configMapItem := .Values.configMaps }}

{{- if $configMapItem.enabled }}
{{ $configMapName := printf "%s-%s" (include "cf-common-0.34.0.names.fullname" $) $configMapIndex }}
---
kind: ConfigMap
apiVersion: v1
metadata:
  ...
  {{- include "cf-common-0.34.0.tplrender" (dict "Values" $configMapItem.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if $configMapItem.annotations }}
  annotations: {{- include "cf-common-0.34.0.tplrender" (dict "Values" $configMapItem.annotations "context" $) | nindent 4 }}
  {{- end }}
  ...
  {{- end }}
data: {{ include "cf-common-0.34.0.tplrender" (dict "Values" $configMapItem.data "context" $) | nindent 2 }}
{{- end }}
