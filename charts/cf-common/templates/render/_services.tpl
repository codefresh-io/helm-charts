...
{{- define "cf-common.v0.0.25.service" }}
...
{{- if and (hasKey $serviceItem "primary") $serviceItem.primary }}
  {{- $serviceName = include "cf-common.v0.0.25.names.fullname" $ -}}
{{- else }}
  {{- $serviceName = printf "%s-%s" (include "cf-common.v0.0.25.names.fullname" $) $serviceIndex -}}
{{- end }}
...
metadata:
  ...
  {{- include "cf-common.v0.0.25.tplrender" (dict "Values" $serviceItem.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if $serviceItem.annotations }}
  annotations: {{- include "cf-common.v0.0.25.tplrender" (dict "Values" $serviceItem.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  type: {{ $serviceType }}
  ...
  ports:
    ...
    - port: {{ required (printf "service.%s.ports.%s.port number is required!" $serviceIndex $portName) $portItem.port }}
      ...
      {{- end }}
    {{- end }}
  selector: {{ include "cf-common.v0.0.25.labels.matchLabels" $ | nindent 4 }}
    {{- if $serviceItem.extraSelectorLabels }}
  {{- include "cf-common.v0.0.25.tplrender" (dict "Values" $serviceItem.extraSelectorLabels "context" $) | nindent 4 }}
    {{- end }}
  {{- end }}
{{- end }}
