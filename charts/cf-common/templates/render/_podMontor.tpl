...
{{- define "cf-common-0.30.0.podMonitor" -}}

{{- range $podMonitorIndex, $podMonitorItem := .Values.podMonitor -}}

{{- $podMonitorName := include "cf-common-0.30.0.names.fullname" $ -}}
...
metadata:
  ...
  {{- include "cf-common-0.30.0.tplrender" (dict "Values" $podMonitorItem.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if $podMonitorItem.annotations }}
  annotations: {{- include "cf-common-0.30.0.tplrender" (dict "Values" $podMonitorItem.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  selector:
    {{- if $podMonitorItem.selector }}
    {{- include "cf-common-0.30.0.tplrender" (dict "Values" $podMonitorItem.selector "context" $) | nindent 4 }}
    {{- else }}
    matchLabels:
  ...
  namespaceSelector:
    {{- include "cf-common-0.30.0.tplrender" (dict "Values" $podMonitorItem.namespaceSelector "context" $) | nindent 6 }}
  {{- else }}
  namespaceSelector:
    matchNames:
      - {{ $.Release.Namespace }}
