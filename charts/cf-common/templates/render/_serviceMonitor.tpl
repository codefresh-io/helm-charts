...
{{- define "cf-common-0.12.2.serviceMonitor" -}}

{{- range $serviceMonitorIndex, $serviceMonitorItem := .Values.serviceMonitor -}}

{{- $serviceMonitorName := include "cf-common-0.12.2.names.fullname" $ -}}
...
metadata:
  ...
  {{- include "cf-common-0.12.2.tplrender" (dict "Values" $serviceMonitorItem.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if $serviceMonitorItem.annotations }}
  annotations: {{- include "cf-common-0.12.2.tplrender" (dict "Values" $serviceMonitorItem.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  selector:
    {{- if $serviceMonitorItem.selector }}
    {{- include "cf-common-0.12.2.tplrender" (dict "Values" $serviceMonitorItem.selector "context" $) | nindent 4 }}
    {{- else }}
    matchLabels:
      {{- include "cf-common-0.12.2.labels.matchLabels" $ | nindent 6 }}
    {{- end }}
  {{- if $serviceMonitorItem.namespaceSelector }}
  namespaceSelector:
    {{- include "cf-common-0.12.2.tplrender" (dict "Values" $serviceMonitorItem.namespaceSelector "context" $) | nindent 6 }}
  {{- else }}
  namespaceSelector:
    matchNames:
      - {{ $.Release.Namespace }}
