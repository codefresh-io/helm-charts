...
*/}}
{{-  define "cf-common-0.3.0.container" -}}

{{/* Restoring root $ context */}}
{{- $ := .context -}}

- name: {{ include "cf-common-0.3.0.names.fullname" $ }}
  image: {{ include "cf-common-0.3.0.image.name" (dict "image" .Values.image "context" $) }}
  ...
  {{- with .Values.command }}
    ...
    {{- end }}
  command: {{- include "cf-common-0.3.0.tplrender" (dict "Values" . "context" $) | nindent 2 }}
  {{- end }}
  ...
  envFrom:
    {{- with .Values.envFrom }}
      ...
      {{- end }}
      {{- include "cf-common-0.3.0.tplrender" (dict "Values" . "context" $) | trim | nindent 4 }}
    {{- end }}
    {{- range $secretName, $_ := $.Values.secrets }}
    - secretRef:
        name: {{ printf "%s-%s" (include "cf-common-0.3.0.names.fullname" $) $secretName }}
    {{- end }}
  {{- end }}
...
*/}}
  ...
  {{- $mergedEnv = merge $mergedEnv $.Values.global.env }}
    {{- end }}
  env:
  {{- include "cf-common-0.3.0.env-vars" (dict "Values" $mergedEnv "context" $) | trim | nindent 2 }}
  {{- end }}
  ...
  volumeMounts:
  {{- include "cf-common-0.3.0.volumeMounts" (dict "Values" . "context" $) | trim | nindent 2 }}
  {{- end }}
