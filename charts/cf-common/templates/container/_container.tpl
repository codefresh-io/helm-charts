...
Usage:
{{ include "cf-common-0.35.0.container" (dict "Values" .Values.container "context" $) }}
*/}}
{{-  define "cf-common-0.35.0.container" -}}

{{/* Restoring root $ context */}}
{{- $ := .context -}}

{{- $containerName := include "cf-common-0.35.0.names.fullname" $ -}}
{{- if and (hasKey .Values "nameOverride") .Values.nameOverride }}
{{- $containerName = include "cf-common-0.35.0.tplrender" (dict "Values" .Values.nameOverride "context" $) -}}
{{- end }}

- name: {{ $containerName }}
  image: {{ (include "cf-common-0.35.0.tplrender" (dict "Values" (include "cf-common-0.35.0.image.name" (dict "image" .Values.image "context" $)) "context" $)) }}
  ...
  {{- with .Values.command }}
    ...
    {{- end }}
  command: {{- include "cf-common-0.35.0.tplrender" (dict "Values" . "context" $) | nindent 2 }}
  {{- end }}
  ...
  envFrom:
    {{- with .Values.envFrom }}
      ...
      {{- end }}
      {{- include "cf-common-0.35.0.tplrender" (dict "Values" . "context" $) | trim | nindent 4 }}
    {{- end }}
    {{- range $secretIndex, $secretItem := $.Values.secrets }}
      {{- if ne $secretItem.envFrom false }}
      {{- if $secretItem.enabled }}
        {{- $secretName := "" }}
        {{- if $secretItem.nameOverride }}
          {{- $secretName = $secretItem.nameOverride }}
        {{- else }}
          {{- $secretName = printf "%s-%s" (include "cf-common-0.35.0.names.fullname" $) $secretIndex }}
        {{- end }}
    - secretRef:
        name: {{ $secretName }}
      {{- end }}
...
*/}}
  ...
  {{- $mergedEnv = merge $mergedEnv $.Values.global.env }}
    {{- end }}
  env:
  {{- include "cf-common-0.35.0.env-vars" (dict "Values" $mergedEnv "context" $) | trim | nindent 2 }}

    {{- if .Values.extraEnv }}
  {{- include "cf-common-0.35.0.env-vars" (dict "Values" .Values.extraEnv "context" $) | trim | nindent 2 }}
    {{- end }}

  {{- end }}

  {{- include "cf-common-0.35.0.ports" $ | trim | nindent 2 }}
...
*/}}
  ...
  {{- $mergedVolumeMounts = mergeOverwrite $mergedVolumeMounts $.Values.volumeMounts }}
    {{- end }}
  volumeMounts: {{ include "cf-common-0.35.0.volumeMounts" (dict "Values" $mergedVolumeMounts "context" $) | trim | nindent 2 }}

  {{- with .Values.probes }}
  {{- include "cf-common-0.35.0.probes" . | trim | nindent 2 }}
  {{- end }}
