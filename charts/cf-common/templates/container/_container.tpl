...
Usage:
{{ include "cf-common-0.17.1.container" (dict "Values" .Values.container "context" $) }}
*/}}
{{-  define "cf-common-0.17.1.container" -}}

{{/* Restoring root $ context */}}
{{- $ := .context -}}

{{- $containerName := include "cf-common-0.17.1.names.fullname" $ -}}
{{- if and (hasKey .Values "nameOverride") .Values.nameOverride }}
{{- $containerName = include "cf-common-0.17.1.tplrender" (dict "Values" .Values.nameOverride "context" $) -}}
{{- end }}

- name: {{ $containerName }}
  image: {{ include "cf-common-0.17.1.image.name" (dict "image" .Values.image "context" $) }}
  ...
  {{- with .Values.command }}
    ...
    {{- end }}
  command: {{- include "cf-common-0.17.1.tplrender" (dict "Values" . "context" $) | nindent 2 }}
  {{- end }}
  ...
  envFrom:
    {{- with .Values.envFrom }}
      ...
      {{- end }}
      {{- include "cf-common-0.17.1.tplrender" (dict "Values" . "context" $) | trim | nindent 4 }}
    {{- end }}
    {{- range $secretName, $secretItem := $.Values.secrets }}
      {{- if $secretItem.enabled }}
    - secretRef:
        name: {{ printf "%s-%s" (include "cf-common-0.17.1.names.fullname" $) $secretName }}
      {{- end }}
    {{- end }}
  {{- end }}
...
*/}}
  ...
  {{- include "cf-common-0.17.1.ports" $ | trim | nindent 2 }}
...
*/}}
  ...
  {{- $mergedVolumeMounts = mergeOverwrite $mergedVolumeMounts $.Values.volumeMounts }}
    {{- end }}
  volumeMounts: {{ include "cf-common-0.17.1.volumeMounts" (dict "Values" $mergedVolumeMounts "context" $) | trim | nindent 2 }}

  {{- with .Values.probes }}
  {{- include "cf-common-0.17.1.probes" . | trim | nindent 2 }}
  {{- end }}
