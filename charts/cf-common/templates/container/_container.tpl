...
Usage:
{{ include "cf-common-0.7.0.container" (dict "Values" .Values.container "context" $) }}
*/}}
{{-  define "cf-common-0.7.0.container" -}}

{{/* Restoring root $ context */}}
{{- $ := .context -}}

{{- $containerName := include "cf-common-0.7.0.names.fullname" $ -}}
{{- if and (hasKey .Values "nameOverride") .Values.nameOverride }}
{{- $containerName = include "cf-common-0.7.0.tplrender" (dict "Values" .Values.nameOverride "context" $) -}}
{{- end }}

- name: {{ $containerName }}
  image: {{ include "cf-common-0.7.0.image.name" (dict "image" .Values.image "context" $) }}
  ...
  {{- with .Values.command }}
    ...
    {{- end }}
  command: {{- include "cf-common-0.7.0.tplrender" (dict "Values" . "context" $) | nindent 2 }}
  {{- end }}
  ...
  envFrom:
    {{- with .Values.envFrom }}
      ...
      {{- end }}
      {{- include "cf-common-0.7.0.tplrender" (dict "Values" . "context" $) | trim | nindent 4 }}
    {{- end }}
    {{- range $secretName, $secretItem := $.Values.secrets }}
      {{- if $secretItem.enabled }}
    - secretRef:
        name: {{ printf "%s-%s" (include "cf-common-0.7.0.names.fullname" $) $secretName }}
      {{- end }}
    {{- end }}
  {{- end }}
...
*/}}
  ...
  {{- $mergedEnv = merge $mergedEnv $.Values.global.env }}
    {{- end }}
  env:
  {{- include "cf-common-0.7.0.env-vars" (dict "Values" $mergedEnv "context" $) | trim | nindent 2 }}
  {{- end }}
