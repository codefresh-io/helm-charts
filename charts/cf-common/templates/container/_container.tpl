...
Usage:
{{ include "cf-common-0.34.0.container" (dict "Values" .Values.container "context" $) }}
*/}}
{{-  define "cf-common-0.34.0.container" -}}
...
{{- $containerName = include "cf-common-0.34.0.tplrender" (dict "Values" .Values.nameOverride "context" $) -}}
{{- end }}

- name: {{ $containerName }}
  image: {{ (include "cf-common-0.34.0.tplrender" (dict "Values" (include "cf-common-0.34.0.image.name" (dict "image" .Values.image "context" $)) "context" $)) }}
  ...
  {{- with .Values.command }}
    ...
    {{- end }}
  command: {{- include "cf-common-0.34.0.tplrender" (dict "Values" . "context" $) | nindent 2 }}
  {{- end }}
  ...
  envFrom:
    {{- with .Values.envFrom }}
      ...
      {{- end }}
      {{- include "cf-common-0.34.0.tplrender" (dict "Values" . "context" $) | trim | nindent 4 }}
    {{- end }}
    {{- range $secretIndex, $secretItem := $.Values.secrets }}
      ...
      {{- if $secretItem.enabled }}
        ...
        {{- else }}
          {{- $secretName = printf "%s-%s" (include "cf-common-0.34.0.names.fullname" $) $secretIndex }}
        {{- end }}
    - secretRef:
        name: {{ $secretName }}
      {{- end }}
...
*/}}
  ...
  {{- $mergedEnv = merge $mergedEnv $.Values.global.env }}
    {{- end }}
  env:
  {{- include "cf-common-0.34.0.env-vars" (dict "Values" $mergedEnv "context" $) | trim | nindent 2 }}

    {{- if .Values.extraEnv }}
  {{- include "cf-common-0.34.0.env-vars" (dict "Values" .Values.extraEnv "context" $) | trim | nindent 2 }}
    {{- end }}

  {{- end }}

  {{- include "cf-common-0.34.0.ports" $ | trim | nindent 2 }}
...
*/}}
  ...
  {{- $mergedVolumeMounts = mergeOverwrite $mergedVolumeMounts $.Values.volumeMounts }}
    {{- end }}
  volumeMounts: {{ include "cf-common-0.34.0.volumeMounts" (dict "Values" $mergedVolumeMounts "context" $) | trim | nindent 2 }}

  {{- with .Values.probes }}
  {{- include "cf-common-0.34.0.probes" . | trim | nindent 2 }}
  {{- end }}
