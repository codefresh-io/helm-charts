...
Usage:
{{ include "cf-common-0.32.0.image.name" (dict "image" .Values.container.image "context" $) }}
*/}}
{{- define "cf-common-0.32.0.image.name" -}}
...
*/}}
{{- define "cf-common-0.32.0.image.pullSecrets" -}}
  {{- $pullSecrets := list }}
    ...
    {{- range $index, $pullSecret := .Values.global.imagePullSecrets -}}
      {{- if and (kindIs "map" $pullSecret ) }}
        {{- if and $pullSecret.name }}
          {{- $pullSecrets = append $pullSecrets (include "cf-common-0.32.0.tplrender" (dict "Values" $pullSecret.name "context" $)) -}}
        {{- end }}
      {{- else }}
        {{- $pullSecrets = append $pullSecrets (include "cf-common-0.32.0.tplrender" (dict "Values" $pullSecret "context" $)) -}}
      {{- end }}
    {{- end -}}
  {{- end -}}
  ...
  {{- if .Values.imagePullSecrets }}
    ...
    {{- range $index, $pullSecret := .Values.imagePullSecrets -}}
      {{- if and (kindIs "map" $pullSecret ) }}
        ...
        {{- end }}
      {{- else }}
        {{- $pullSecrets = append $pullSecrets (include "cf-common-0.32.0.tplrender" (dict "Values" . "context" $)) -}}
      {{- end }}
    {{- end -}}
  {{- end -}}
