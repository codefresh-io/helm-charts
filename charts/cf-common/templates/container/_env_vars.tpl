...
Usage:
env: {{ include "cf-common-0.9.0.env-vars" (dict "Values" .Values.container.env "context" $) | nindent }}
*/}}
{{- define "cf-common-0.9.0.env-vars"}}
{{- $ := .context }}
  {{- if .Values }}
  ...
  {{- end }}
  {{- $env := .Values }}
  {{- $templatedEnv := include "cf-common-0.9.0.tplrender" (dict "Values" $env "context" $) | fromYaml }}
