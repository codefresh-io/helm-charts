...
Usage:
env: {{ include "cf-common-0.15.1.env-vars" (dict "Values" .Values.container.env "context" $) | nindent }}
*/}}
{{- define "cf-common-0.15.1.env-vars"}}
{{- $ := .context }}
  {{- if .Values }}
  ...
  {{- end }}
  {{- $env := .Values }}
  {{- $templatedEnv := include "cf-common-0.15.1.tplrender" (dict "Values" $env "context" $) | fromYaml }}
