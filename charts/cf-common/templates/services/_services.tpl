...
{{- define "cf-common-0.15.2.service" }}

{{- if not (kindIs "map" .Values.service) }}
  {{- fail "ERROR: service block must be a map!" }}
{{- end }}

{{- $primary := include "cf-common-0.15.2.service.primary" (dict "values" .Values.service) -}}
...
{{- else }}
  {{- $serviceName = printf "%s-%s" (include "cf-common-0.15.2.names.fullname" $) $serviceIndex -}}
{{- end }}
...
metadata:
  ...
  {{- include "cf-common-0.15.2.tplrender" (dict "Values" $serviceItem.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if $serviceItem.annotations }}
  annotations: {{- include "cf-common-0.15.2.tplrender" (dict "Values" $serviceItem.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  type: {{ $serviceType }}
  ...
  ports:
    ...
    - port: {{ required (printf "service.%s.ports.%s.port number is required!" $serviceIndex $portName) $portItem.port }}
      ...
      {{- end }}
    {{- end }}
  selector: {{ include "cf-common-0.15.2.labels.matchLabels" $ | nindent 4 }}
    {{- if $serviceItem.extraSelectorLabels }}
  {{- include "cf-common-0.15.2.tplrender" (dict "Values" $serviceItem.extraSelectorLabels "context" $) | nindent 4 }}
    {{- end }}
  {{- end }}
{{- end }}
