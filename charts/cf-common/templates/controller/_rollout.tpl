...
Usage:
{{ include "cf-common.v0.1.0.controller.rollout" . | nindent 0 }}
*/}}
{{- define "cf-common.v0.1.0.controller.rollout" -}}

{{- $strategy := .Values.controller.rollout.strategy -}}
{{- $fullName:= include "cf-common.v0.1.0.names.fullname" . }}

{{- if and (ne $strategy "Canary") (ne $strategy "BlueGreen") -}}
  {{- fail (printf "ERROR: %s is invalid Rollout strategy!" $strategy) -}}
{{- end -}}

{{- $rolloutName := include "cf-common.v0.1.0.names.fullname" . -}}
...
spec:
  ...
  template:
    metadata:
      ...
      annotations:
      {{- include "cf-common.v0.1.0.tplrender" (dict "Values" .Values.podAnnotations "context" $) | nindent 8 }}
      {{- end }}
