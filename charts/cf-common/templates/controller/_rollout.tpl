...
Usage:
{{ include "cf-common-0.2.0.controller.rollout" . | nindent 0 }}
*/}}
{{- define "cf-common-0.2.0.controller.rollout" -}}

{{- $strategy := .Values.controller.rollout.strategy -}}
{{- $fullName:= include "cf-common-0.2.0.names.fullname" . }}

{{- if and (ne $strategy "Canary") (ne $strategy "BlueGreen") -}}
  {{- fail (printf "ERROR: %s is invalid Rollout strategy!" $strategy) -}}
{{- end -}}

{{- $rolloutName := include "cf-common-0.2.0.names.fullname" . -}}
...
metadata:
  ...
  {{- include "cf-common-0.2.0.tplrender" (dict "Values" .Values.controller.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if .Values.controller.annotations }}
  annotations:
  {{- include "cf-common-0.2.0.tplrender" (dict "Values" .Values.controller.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  ...
  {{- end }}
  selector:
    matchLabels: {{ include "cf-common-0.2.0.labels.matchLabels" . | nindent 6 }}
  ...
  template:
    metadata:
      labels: {{ include "cf-common-0.2.0.labels.matchLabels" . | nindent 8 }}
      {{- if .Values.podLabels }}
      {{- include "cf-common-0.2.0.tplrender" (dict "Values" .Values.podLabels "context" $) | nindent 8 }}
      {{- end }}
      {{- if .Values.podAnnotations }}
      annotations:
      {{- include "cf-common-0.2.0.tplrender" (dict "Values" .Values.podAnnotations "context" $) | nindent 8 }}
      {{- end }}
    spec: {{- include "cf-common-0.2.0.controller.pod" . | trim | nindent 6 -}}
{{- end -}}
