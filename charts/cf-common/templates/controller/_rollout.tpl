...
Usage:
{{ include "cf-common-0.5.2.controller.rollout" . }}
*/}}
{{- define "cf-common-0.5.2.controller.rollout" -}}
...
{{- $rolloutName := include "cf-common-0.5.2.names.fullname" . -}}
...
metadata:
  ...
  {{- include "cf-common-0.5.2.tplrender" (dict "Values" .Values.controller.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if .Values.controller.annotations }}
  annotations:
  {{- include "cf-common-0.5.2.tplrender" (dict "Values" .Values.controller.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  ...
  {{- end }}
  selector:
    matchLabels: {{ include "cf-common-0.5.2.labels.matchLabels" . | nindent 6 }}
  ...
  template:
    metadata:
      labels: {{ include "cf-common-0.5.2.labels.matchLabels" . | nindent 8 }}
      {{- if .Values.podLabels }}
      {{- include "cf-common-0.5.2.tplrender" (dict "Values" .Values.podLabels "context" $) | nindent 8 }}
      {{- end }}
      {{- if .Values.podAnnotations }}
      annotations:
      {{- include "cf-common-0.5.2.tplrender" (dict "Values" .Values.podAnnotations "context" $) | nindent 8 }}
      {{- end }}
    spec: {{- include "cf-common-0.5.2.controller.pod" . | trim | nindent 6 -}}
{{- end -}}
