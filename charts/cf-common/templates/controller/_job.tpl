...
Usage:
{{ include "cf-common-0.7.0.controller.job" . }}
*/}}

{{- define "cf-common-0.7.0.controller.job" -}}

---
apiVersion: batch/v1
kind: Job
metadata:
  name: {{ include "cf-common-0.7.0.names.fullname" . }}
...
spec:
  ...
  selector:
    matchLabels: {{- include "cf-common-0.7.0.tplrender" (dict "Values" .Values.controller.job.selector "context" $) | nindent 6 }}
    {{- else }}
  ...
  template:
    metadata:
      ...
      {{- end }}
      {{- with include "cf-common-0.7.0.annotations.podAnnotations" . }}
      annotations:
        {{- . | nindent 8 }}
      {{- end }}
