...
{{- define "cf-common-0.1.0.volumes" -}}
...
- name: {{ $volumeIndex }}
  ...
  {{- $volumeName := printf "%s-%s" (include "cf-common-0.1.0.names.fullname" $) $volumeIndex -}}

  {{- if and (hasKey $volumeItem "existingName") $volumeItem.existingName  }}
  {{- $volumeName = include "cf-common-0.1.0.tplrender" (dict "Values" $volumeItem.existingName "context" $) -}}
  {{- end }}
