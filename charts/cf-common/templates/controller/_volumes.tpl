...
Usage:
volumes: {{ include "cf-common-0.5.2.volumes" (dict "Values" .Values.volumes "context" $) | nindent }}
*/}}

{{- define "cf-common-0.5.2.volumes" -}}
...
- name: {{ $volumeIndex }}
  {{- $volumeName := printf "%s-%s" (include "cf-common-0.5.2.names.fullname" $) $volumeIndex -}}
  ...
  {{- $volumeName = include "cf-common-0.5.2.tplrender" (dict "Values" (coalesce $volumeItem.nameOverride $volumeItem.existingName) "context" $) -}}
  {{- end }}
