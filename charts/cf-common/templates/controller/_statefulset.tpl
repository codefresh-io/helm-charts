...
Usage:
{{ include "cf-common-0.18.1.controller.statefulset" . }}
*/}}

{{- define "cf-common-0.18.1.controller.statefulset" -}}
...
{{- $stsName := include "cf-common-0.18.1.names.fullname" . -}}
...
metadata:
  ...
  {{- include "cf-common-0.18.1.tplrender" (dict "Values" .Values.controller.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if .Values.controller.annotations }}
  annotations:
  {{- include "cf-common-0.18.1.tplrender" (dict "Values" .Values.controller.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  ...
  podManagementPolicy: {{ default "OrderedReady" .Values.controller.podManagementPolicy }}
  selector:
    matchLabels: {{ include "cf-common-0.18.1.labels.matchLabels" . | nindent 6 }}
  serviceName: {{ include "cf-common-0.18.1.names.fullname" . }}
  updateStrategy:
    type: {{ $strategy }}
  ...
  template:
    metadata:
      labels: {{ include "cf-common-0.18.1.labels.matchLabels" . | nindent 8 }}
      {{- if .Values.podLabels }}
      {{- include "cf-common-0.18.1.tplrender" (dict "Values" .Values.podLabels "context" $) | nindent 8 }}
      {{- end }}
      {{- if .Values.podAnnotations }}
      annotations:
      {{- include "cf-common-0.18.1.tplrender" (dict "Values" .Values.podAnnotations "context" $) | nindent 8 }}
      {{- end }}
    spec: {{- include "cf-common-0.18.1.controller.pod" . | trim | nindent 6 }}
  ...
  {{- range $claimIndex, $claimItem := .Values.volumeClaimTemplates }}
    ...
    - metadata:
        ...
        labels: {{- include "cf-common-0.18.1.tplrender" (dict "Values" . "context" $) | nindent 10 }}
        {{- end }}
        {{- with ($claimItem.annotations | default dict) }}
        annotations: {{- include "cf-common-0.18.1.tplrender" (dict "Values" . "context" $) | nindent 10 }}
        {{- end }}
      spec:
        ...
        {{ include "cf-common-0.18.1.storageclass" ( dict "persistence" $claimItem "context" $) }}
      {{- end }}
    {{- end }}
{{- end -}}
