...
Usage:
{{ include "cf-common-0.12.1.controller.statefulset" . }}
*/}}

{{- define "cf-common-0.12.1.controller.statefulset" -}}
...
{{- $stsName := include "cf-common-0.12.1.names.fullname" . -}}
...
spec:
  ...
  podManagementPolicy: {{ default "OrderedReady" .Values.controller.podManagementPolicy }}
  selector:
    matchLabels: {{ include "cf-common-0.12.1.labels.matchLabels" . | nindent 6 }}
  ...
  template:
    metadata:
      ...
      annotations:
      {{- include "cf-common-0.12.1.tplrender" (dict "Values" .Values.podAnnotations "context" $) | nindent 8 }}
      {{- end }}
  ...
  {{- range $claimIndex, $claimItem := .Values.volumeClaimTemplates }}
    ...
    - metadata:
        ...
        labels: {{- include "cf-common-0.12.1.tplrender" (dict "Values" . "context" $) | nindent 10 }}
        {{- end }}
        {{- with ($claimItem.annotations | default dict) }}
        annotations: {{- include "cf-common-0.12.1.tplrender" (dict "Values" . "context" $) | nindent 10 }}
        {{- end }}
      spec:
        ...
        {{ include "cf-common-0.12.1.storageclass" ( dict "persistence" $claimItem "context" $) }}
      {{- end }}
    {{- end }}
{{- end -}}
