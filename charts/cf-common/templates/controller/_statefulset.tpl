...
Usage:
{{ include "cf-common-0.8.0.controller.statefulset" . }}
*/}}

{{- define "cf-common-0.8.0.controller.statefulset" -}}
...
{{- $stsName := include "cf-common-0.8.0.names.fullname" . -}}
...
metadata:
  ...
  {{- include "cf-common-0.8.0.tplrender" (dict "Values" .Values.controller.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if .Values.controller.annotations }}
  annotations:
  {{- include "cf-common-0.8.0.tplrender" (dict "Values" .Values.controller.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  ...
  serviceName: {{ include "cf-common-0.8.0.names.fullname" . }}
  updateStrategy:
    type: {{ $strategy }}
  ...
  template:
    metadata:
      labels: {{ include "cf-common-0.8.0.labels.matchLabels" . | nindent 8 }}
      {{- if .Values.podLabels }}
      {{- include "cf-common-0.8.0.tplrender" (dict "Values" .Values.podLabels "context" $) | nindent 8 }}
      {{- end }}
      {{- if .Values.podAnnotations }}
      annotations:
      {{- include "cf-common-0.8.0.tplrender" (dict "Values" .Values.podAnnotations "context" $) | nindent 8 }}
      {{- end }}
    spec: {{- include "cf-common-0.8.0.controller.pod" . | trim | nindent 6 }}
  ...
  {{- range $claimIndex, $claimItem := .Values.volumeClaimTemplates }}
    ...
    - metadata:
        ...
        labels: {{- include "cf-common-0.8.0.tplrender" (dict "Values" . "context" $) | nindent 10 }}
        {{- end }}
        {{- with ($claimItem.annotations | default dict) }}
        annotations: {{- include "cf-common-0.8.0.tplrender" (dict "Values" . "context" $) | nindent 10 }}
        {{- end }}
      spec:
        ...
        {{ include "cf-common-0.8.0.storageclass" ( dict "persistence" $claimItem "context" $) }}
      {{- end }}
    {{- end }}
{{- end -}}
