...
Usage:
{{ include "cf-common-0.9.3.controller.cronjob" . }}
*/}}

{{- define "cf-common-0.9.3.controller.cronjob" -}}
...
spec:
  ...
  jobTemplate:
    spec:
      ...
      template:
        metadata:
          labels: {{ include "cf-common-0.9.3.labels.matchLabels" . | nindent 12 }}
          {{- if .Values.podLabels }}
          {{- include "cf-common-0.9.3.tplrender" (dict "Values" .Values.podLabels "context" $) | nindent 12 }}
          {{- end }}
          {{- with include "cf-common-0.9.3.annotations.podAnnotations" . }}
          annotations:
            {{- . | nindent 12 }}
          {{- end }}
        spec: {{- include "cf-common-0.9.3.controller.pod" . | trim | nindent 10 -}}
