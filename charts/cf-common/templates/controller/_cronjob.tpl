...
Usage:
{{ include "cf-common-0.8.1.controller.cronjob" . }}
*/}}

{{- define "cf-common-0.8.1.controller.cronjob" -}}
...
spec:
  ...
  jobTemplate:
    spec:
      ...
      template:
        metadata:
          labels: {{ include "cf-common-0.8.1.labels.matchLabels" . | nindent 12 }}
          {{- if .Values.podLabels }}
          {{- include "cf-common-0.8.1.tplrender" (dict "Values" .Values.podLabels "context" $) | nindent 12 }}
          {{- end }}
          {{- with include "cf-common-0.8.1.annotations.podAnnotations" . }}
          annotations:
            {{- . | nindent 12 }}
          {{- end }}
        spec: {{- include "cf-common-0.8.1.controller.pod" . | trim | nindent 10 -}}
