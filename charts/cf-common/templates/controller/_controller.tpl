...
Usage:
{{- include "cf-common-0.5.0.controller" . -}}
*/}}

{{- define "cf-common-0.5.0.controller" -}}
...
{{- if .Values.controller.enabled -}}
  ...
  {{- if eq .Values.controller.type "rollout" }}
    {{ include "cf-common-0.5.0.controller.rollout" . | nindent 0 }}
    {{- if .Values.controller.rollout.analysisTemplate.enabled }}
    {{ include "cf-common-0.5.0.controller.analysis-template" . | nindent 0 }}
    {{- end }}
  {{- else if eq .Values.controller.type "deployment" }}
    {{ include "cf-common-0.5.0.controller.deployment" . | nindent 0 }}
  {{- else if eq .Values.controller.type "job" }}
    {{ include "cf-common-0.5.0.controller.job" . | nindent 0 }}
  {{- else if eq .Values.controller.type "cronjob" }}
    {{ include "cf-common-0.5.0.controller.cronjob" . | nindent 0 }}
  {{- else }}
