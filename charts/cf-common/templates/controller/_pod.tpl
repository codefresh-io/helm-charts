...
Usage:
{{ include "cf-common-0.29.0.controller.pod" . }}
*/}}
{{- define "cf-common-0.29.0.controller.pod" -}}

{{- include "cf-common-0.29.0.image.pullSecrets" . }}

serviceAccountName: {{ include "cf-common-0.29.0.names.serviceAccountName" . }}
...
initContainers:
  {{- range $initContainerIndex, $initContainerItem := .Values.initContainers }}
    ...
    {{- include "cf-common-0.29.0.container" (dict "Values" $initContainerItem "context" $) | trim | nindent 0 }}
    {{- end }}
  {{- end }}
{{- end }}

{{- with .Values.container }}
containers: {{ include "cf-common-0.29.0.container" (dict "Values" . "context" $) | trim | nindent 0 }}
{{- end }}
{{- with .Values.additionalContainers }}
{{ toYaml . | nindent 0 }}
{{- end }}

volumes:  {{ include "cf-common-0.29.0.volumes" (dict "Values" .Values.volumes "context" $) | trim | nindent 0 }}

{{- with .Values.extraVolumes }}
{{ include "cf-common-0.29.0.volumes" (dict "Values" . "context" $) | trim }}
{{- end }}
...
{{- end }}
{{- with $nodeSelector | default $globalNodeSelector }}
...
{{- end }}
{{- with $tolerations | default $globalTolerations}}
...
{{- end }}
{{- with $affinity | default $globalAffinity }}
affinity: {{ toYaml . | nindent 2 }}
{{- end }}

{{- $topologySpreadConstraints := .Values.topologySpreadConstraints | default list }}
{{- $globalTopologySpreadConstraints := .Values.global.topologySpreadConstraints | default list }}
{{- if or (not (kindIs "slice" $tolerations)) (not (kindIs "slice" $globalTolerations)) }}
  {{- fail "ERROR: topologySpreadConstraints block must be a list!" }}
{{- end }}
{{- with $topologySpreadConstraints | default $globalTopologySpreadConstraints }}
topologySpreadConstraints: {{- include "cf-common-0.29.0.tplrender" (dict "Values" . "context" $) | nindent 2 }}
{{- end }}
