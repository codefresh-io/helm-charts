...
*/}}
{{- define "cf-common-0.5.0.ingress" -}}
  {{- range $name, $ingress := .Values.ingress -}}
    {{- if $ingress.enabled -}}
      {{- $ingressValues := $ingress -}}

      {{/* set defaults */}}
      {{- if and (not $ingressValues.nameOverride) (ne $name (include "cf-common-0.5.0.ingress.primary" $)) -}}
        {{- $_ := set $ingressValues "nameOverride" $name -}}
      {{- end -}}

      {{- $_ := set $ingressValues "name" $name -}}
      {{- $_ := set $ "ObjectValues" (dict "ingress" $ingressValues) -}}
      {{- include "cf-common-0.5.0.ingress.render" $ | nindent 0 -}}
    {{- end -}}
  {{- end -}}
{{- end -}}
...
metadata:
  ...
  {{- include "cf-common-0.5.0.tplrender" (dict "Values" $ingressItem.labels "context" $) | nindent 4 }}
  {{- end }}
  {{- if $ingressItem.annotations }}
  annotations: {{- include "cf-common-0.5.0.tplrender" (dict "Values" $ingressItem.annotations "context" $) | nindent 4 }}
  {{- end }}
spec:
  ...
  tls:
    ...
    - hosts:
        {{- range .hosts }}
        - {{ include "cf-common-0.5.0.tplrender" (dict "Values" . "context" $) | quote }}
        {{- end }}
      {{- if .secretName }}
      secretName: {{ include "cf-common-0.5.0.tplrender" (dict "Values" .secretName "context" $) | quote}}
      {{- end }}
    {{- end }}
  {{- end }}
  rules:
  ...
  {{- range $ingressItem.hosts }}
    - host: {{ include "cf-common-0.5.0.tplrender" (dict "Values" .host "context" $) | quote }}
      http:
        paths:
          ...
          {{- range .paths }}
            ...
            {{- end }}
          - path: {{ include "cf-common-0.5.0.tplrender" (dict "Values" .path "context" $) | quote }}
            pathType: {{ default "ImplementationSpecific" .pathType }}
            backend:
              service:
                name: {{ include "cf-common-0.5.0.tplrender" (dict "Values" $service "context" $) }}
                port:
